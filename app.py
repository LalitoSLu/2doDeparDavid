...
try:
    ...
    # Pre-cargar modelos de ML en memoria
    modelos_ml = {}
    for key in ["regresion_logistica", "random_forest", "xgboost", "red_neuronal_simple"]:
        try:
            modelos_ml[key] = joblib.load(f"modelo_{key}.joblib")
        except Exception as e:
            print(f"Error cargando {key}: {e}")
    scaler_ml = joblib.load("scaler.joblib")
    
    print("Datos, métricas y modelos ML cargados exitosamente.")
except Exception as e:
...
def get_kpis():
    ...
    # Determinar el motivo principal del riesgo para el estado
    df_max_estado = df_filtered[df_filtered["Estado"] == estado_max_tasa]
    max_pm25 = df_max_estado["PM25"].mean()
    max_temp = df_max_estado["Temperatura"].mean()
    max_vuln = df_max_estado["Poblacion_Vulnerable_Pct"].mean()
    
    motivo = "Múltiples factores ambientales"
    if max_pm25 > 35:
        motivo = f"Contaminación severa (PM2.5: {max_pm25:.1f})"
    elif max_temp < 12:
        motivo = f"Bajas temperaturas ({max_temp:.1f}°C)"
    elif max_vuln > 18:
        motivo = f"Alta vulnerabilidad ({max_vuln:.1f}%)"
    elif max_pm25 > 25:
        motivo = f"Calidad de aire deficiente (PM2.5: {max_pm25:.1f})"
...
def predict():
    data = request.json
    modelo_key = data.get("modelo", "Regresion_Logistica")
    features = data.get("features", {})
    
    try:
        clf = modelos_ml.get(modelo_key.lower())
        scaler = scaler_ml
