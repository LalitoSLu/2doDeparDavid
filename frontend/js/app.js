...
const layoutConfig = {
    ...
    yaxis: { showgrid: true, gridcolor: '#e2e8f0', zeroline: false, color: '#64748b' }
};
...
async function loadDashboardData() {
    ...
    try {
        ...
        if(kpiRes.ok) {
            ...
            if (document.getElementById('kpi-max-reason')) {
                document.getElementById('kpi-max-reason').innerText = `Motivo: ${kpi.motivo_max_tasa}`;
            }
        }
        ...
        if(timeRes.ok) {
            ...
            const openVals = [];
            const closeVals = tData.casos_100k;
            const highVals = [];
            const lowVals = [];
            
            // Generate synthetic OHLC data based on the trend to create a realistic Candlestick chart
            for(let i = 0; i < tData.casos_100k.length; i++) {
                const c = tData.casos_100k[i];
                let o;
                if (i === 0) {
                    o = c * 0.95; 
                } else {
                    o = tData.casos_100k[i-1];
                }
                openVals.push(o);
                highVals.push(Math.max(o, c) * (1 + (Math.random() * 0.05)));
                lowVals.push(Math.min(o, c) * (1 - (Math.random() * 0.05)));
            }
            ...
            const traceHist = {
                ...
                close: closeVals,
                type: 'candlestick',
                name: 'Fluctuación Mensual',
                increasing: {line: {color: '#ef4444'}}, // Red = Cases going up (Bad)
                decreasing: {line: {color: '#22c55e'}}  // Green = Cases going down (Good)
            };
        ...
        if(pollRes.ok) {
            ...
            const trace = {
                values: pData.pm25,
                labels: pData.estados,
                type: 'pie',
                hole: 0.6,
                marker: { colors: colors.slice(0, pData.estados.length) },
                textinfo: 'none',
                hoverinfo: 'label+percent+value'
            };
            const layoutRight = { ...layoutConfig, showlegend: false, margin: { t: 10, r: 10, l: 10, b: 10 } };
...
async function loadMapData(forceYear = null) {
    ...
    const trace = {
        ...
        zmin: 0,
        zmax: 1,
        colorbar: { title: 'Nivel Riesgo', tickfont: {color: '#1e293b'} },
        hovertemplate: '<b>%{location}</b><br>Riesgo Promedio: %{z:.2f}<br><extra></extra>'
    };
    
    const layout = {
        mapbox: { style: "carto-positron", center: { lon: -102.5528, lat: 23.6345 }, zoom: 4.5 },
        paper_bgcolor: 'rgba(0,0,0,0)',
        plot_bgcolor: 'rgba(0,0,0,0)',
...
async function runPrediction() {
    ...
    if(res.ok) {
        ...
        const layoutRadar = {
            polar: {
                radialaxis: { visible: true, range: [0, 100], color: '#e2e8f0' },
                angularaxis: { color: '#64748b' },
        ...
        const recPanel = document.getElementById('sim-recommendation-panel');
        recPanel.style.display = 'block';
        let recHTML = '<div class="panel-title" style="margin-top:20px;">Recomendaciones Clínicas IA</div>';
        
        if(pm25 > 40) {
            recHTML += `<div class="rec-item rec-danger"><i class="fa-solid fa-triangle-exclamation"></i><div><strong>Alerta por PM2.5 Crítico (${pm25} µg/m³):</strong> Suspender actividades al aire libre inmediatamente. Alto riesgo de crisis asmáticas.</div></div>`;
        } else if(pm25 > 25) {
            recHTML += `<div class="rec-item rec-warning"><i class="fa-solid fa-triangle-exclamation"></i><div><strong>Precaución por PM2.5 (${pm25} µg/m³):</strong> Calidad del aire regular. Grupos vulnerables deben usar cubrebocas N95.</div></div>`;
        } else {
            recHTML += `<div class="rec-item rec-success"><i class="fa-solid fa-circle-check"></i><div><strong>Calidad del aire óptima:</strong> Los niveles de material particulado son seguros.</div></div>`;
        }
        
        if(temp < 10) {
            recHTML += `<div class="rec-item rec-danger"><i class="fa-solid fa-snowflake"></i><div><strong>Temperaturas Congelantes (${temp}°C):</strong> Preparar camas de urgencias por probable repunte de neumonía e influenza.</div></div>`;
        }
        
        if(vuln > 20 && data.riesgo_alto === 1) {
            recHTML += `<div class="rec-item rec-danger"><i class="fa-solid fa-hospital"></i><div><strong>Demografía Vulnerable Crítica:</strong> Desplegar brigadas de vacunación inmediatas debido al alto porcentaje de riesgo.</div></div>`;
        }
        
        if(data.riesgo_alto === 0 && temp >= 10 && pm25 <= 25) {
            recHTML += `<div class="rec-item rec-info"><i class="fa-solid fa-notes-medical"></i><div><strong>Pronóstico Estable:</strong> No se requieren medidas extraordinarias de contingencia por el momento.</div></div>`;
        }
        
        recPanel.innerHTML = recHTML;
        
        showToast('Predicción y Recomendaciones generadas con éxito.', 'purple');
    }
}
...
async function loadMetricsData() {
    try {
        ...
        const traceF1 = { x: labels, y: f1Scores, name: 'Puntuación F1', type: 'bar', marker: {color: '#8b5cf6'} };
        const traceAUC = { x: labels, y: aucScores, name: 'AUC-ROC', type: 'bar', marker: {color: '#3b82f6'} };
        const traceAcc = { x: labels, y: accScores, name: 'Exactitud', type: 'bar', marker: {color: '#10b981'} };
        
        Plotly.newPlot('metrics-bar-chart', [traceAcc, traceF1, traceAUC], { ...layoutConfig, title: 'Comparación Global de Métricas', barmode: 'group', yaxis: { range: [0.8, 1.0], gridcolor: '#e2e8f0' } }, {responsive: true});
